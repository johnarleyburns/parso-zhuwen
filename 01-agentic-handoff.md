...
> **Decision record (MC-4, supersedes the two-repo table below):** the project ships as a
> single **monorepo `parso-zhuwen`** with `factory/` (Go) and `ios/` (SwiftUI) side by side
> (plus `plans/` and the root design docs). Rationale: simpler for a solo architect — one
> clone, one PR, one CI matrix — and fixture vendoring becomes an in-repo **path**
> (`factory` regenerates `ios/Fixtures/`) rather than a cross-repo sync. The row labels
> `parso-zhuwen-factory` / `parso-zhuwen-ios` below now name the `factory/` and `ios/`
> **directories**, not separate repositories. The `@main` app target lives in `ios/App/`
> (XcodeGen, MC-1).
...
directory under `Fixtures/` (regenerated by `make -C factory fixtures`) and treated as
golden inputs.
