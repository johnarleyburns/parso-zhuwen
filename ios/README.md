...
- `fixture-a2-v0.zpack` + `zhuwen-dev.pub` — positive pack + verify key. Each story now
  ships audio (`audio/<id>.opus`) + word-level `alignment` rows (CP-06; the real CosyVoice
  render landed in CP-09c as the build-time `internal/tts` stage, with a deterministic stub
  for hermetic fixtures). The pack also ships Foundations F0 `foundations_card` rows (CP-08a)
  that resolve to provenanced images and drive the on-device Foundations engine + M14 UI.
- `real-render-alignment.json` — a ~3-minute real-render alignment sample (non-uniform word
  timings) used by `KaraokeDriftTests` to re-verify highlight drift < 120 ms against the real
  CosyVoice timings (CP-09c Part C; pre-push-gated).
...
- CP-09c: real render + measured budgets. Story audio is now CosyVoice-rendered Opus with real
  word-level alignment (factory `internal/tts`, build-time only; deterministic stub for hermetic
  fixtures). `KaraokeDriftTests` re-verifies highlight drift < 120 ms against a real-render
  alignment sample (`Fixtures/real-render-alignment.json`, pre-push). Pack sizes are asserted
  against NFR-3/NFR-4 by a build-failing `pack.MeasureBudget` test in the factory. No app-side
  change (I3: app still only reads shipped audio + timings); `make audit` stays green. ✅
